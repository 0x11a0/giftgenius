...
# Ensure reproducibility for test results
random.seed(42)
...
from algorithms.simulated_annealing import genetic_algorithm_with_annealing
...
class TestSimulatedAnnealing(unittest.TestCase):
    ...
    def test_genetic_algorithm_with_annealing_sufficient_budget(self):
        # Test the algorithm with a budget that allows multiple items
        budget = 100
        best_bundle, best_satisfaction = genetic_algorithm_with_annealing(self.products, self.user_tags, budget)
        ...
        # Assert that we get a positive satisfaction score
        self.assertGreater(best_satisfaction, 0, "Expected a positive satisfaction score.")

        # Print results for verification
        print("\nTest Output - Genetic Algorithm with Simulated Annealing (Sufficient Budget)")
    ...
    def test_genetic_algorithm_with_annealing_low_budget(self):
        ...
        # Assert that we get a positive satisfaction score and only one item selected
        self.assertGreater(best_satisfaction, 0, "Expected a positive satisfaction score.")
        self.assertEqual(len(best_bundle), 1, "Expected only one item in the bundle due to low budget.")

        # Print results for verification
        print("\nTest Output - Genetic Algorithm with Simulated Annealing (Low Budget)")
        for product in best_bundle:
            print(f"- {product.name} (${product.price}) - Satisfaction: {product.base_satisfaction}")
        print(f"Total Satisfaction Score: {best_satisfaction}")
        print(f"Total Cost: {total_cost}\n")

    def test_genetic_algorithm_with_annealing_high_budget(self):
        ...
        # Print results for verification
        print("\nTest Output - Genetic Algorithm with Simulated Annealing (High Budget)")
        for product in best_bundle:
            print(f"- {product.name} (${product.price}) - Satisfaction: {product.base_satisfaction}")
        print(f"Total Satisfaction Score: {best_satisfaction}")
        print(f"Total Cost: {total_cost}\n")
